...
export class WaiterService {
  ...
  async createOrder(
    ...
    for (const itemDto of items) {
      ...
      orderItemsCreateInput.push({
        ...
        notes: itemDto.notes,
        menuItem: {
          // Correct way to connect to an existing MenuItem
  ...
  async getWaiterDashboardStats(waiter: User) {
    ...
    const pendingOrders = await this.prisma.order.count({
      where: {
        waiterId: waiter.id,
        status: {
          in: [OrderStatus.RECEIVED, OrderStatus.IN_PROCESS, OrderStatus.READY],
        },
        isPaid: false,
      },
    });
  ...
  async updateOrderStatusByWaiter(
    orderId: string,
    action: 'send_to_kitchen' | 'served' | 'proceed_payment',
    waiter: User,
  ): Promise<Order> {
    const order = await this.prisma.order.findUnique({
      where: { id: orderId },
    ...
    if (action === 'send_to_kitchen') {
      if (order.status !== OrderStatus.RECEIVED)
        throw new BadRequestException(
          'Order must be in RECEIVED status to send to kitchen.',
        );
      newStatus = OrderStatus.IN_QUEUE;
    } else if (action === 'served') {
      if (order.status !== OrderStatus.READY)
        throw new BadRequestException(
          'Order must be in READY status to be served.',
        );
      newStatus = OrderStatus.DELIVERED;
    } else if (action === 'proceed_payment') {
      if (order.status !== OrderStatus.DELIVERED)
        throw new BadRequestException(
          'Order must be in DELIVERED status to proceed payment.',
        );
  ...
  async completePayment(
    ...
    if (order.status !== 'PENDING_PAYMENT')
      throw new BadRequestException(
        'Order must be in PENDING_PAYMENT status to complete payment.',
      );
  ...
  async findPendingPaymentOrders(waiter: User): Promise<Order[]> {
    return this.prisma.order.findMany({
      where: {
        waiterId: waiter.id,
  ...
  async findPendingPaymentOrderById(orderId: string, waiter: User): Promise<Order | null> {
    const order = await this.prisma.order.findUnique({
      where: { id: orderId },
      include: {
        table: true,
    ...
    if (order.waiterId !== waiter.id) throw new ForbiddenException('You are not authorized to view this order.');
    if (order.status !== OrderStatus.PENDING_PAYMENT) throw new BadRequestException('Order is not pending payment.');
    return order;
  }

  async recentTransactions(waiter: User): Promise<Order[]> {
    return this.prisma.order.findMany({
      where: {
        waiterId: waiter.id,
      },
